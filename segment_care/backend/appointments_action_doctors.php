...
// get total count
$pstmt = $db->prepare('SELECT * FROM appointments');
$pstmt->execute(array());
...
$sql_query .= 'WHERE fullname LIKE "%'.$search_value.'%"  OR  email LIKE "%'.$search_value.'%" OR status LIKE "%'.$search_value.'%"';
...
<button style='display:nonex;' id='statushide_$id' title='Click to Approve Appointments' class='report_css updates_btn btn_call_update'
 data-id='$id'
 data-userid='$id'
 >
...
<button type="button"  data-id="'. intval($row["id"]).'"  data-pang_id="'. $row["id"].'" class="btn btn-danger btn-xs delete_btnx btn_call_delete">Delete Data</button>';
